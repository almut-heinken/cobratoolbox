...
growthRequirements = readInputTableForPipeline([inputDataFolder filesep 'GrowthRequirementsTable_tmp.txt']);
...
if contains(version,'(R202') % for Matlab R2020a and newer
    if any(cell2mat(growthRequirements(mInd,2:end))==-1) || any(cell2mat(growthRequirements(mInd,2:end))==1)
        experimentalDataExists=1;
    else
        experimentalDataExists=0;
    end
else
    if any(str2double(growthRequirements(mInd,2:end))==-1) || any(str2double(growthRequirements(mInd,2:end))==1)
        experimentalDataExists=1;
    else
        experimentalDataExists=0;
    end
...
if experimentalDataExists
    exchangesToFind=growthRequirements(1,2:end);

    % delete metabolites that are not essential according to experimental data
    if contains(version,'(R202') % for Matlab R2020a and newer
        nonessentialData = exchangesToFind(find(cell2mat(growthRequirements(mInd,2:end))==-1));
    else
        nonessentialData = exchangesToFind(find(str2double(growthRequirements(mInd,2:end))==-1));
    end
    [C,IA,IB] = intersect(growthExchanges(:,1),nonessentialData,'stable');
    growthExchanges(IA,:)=[];
